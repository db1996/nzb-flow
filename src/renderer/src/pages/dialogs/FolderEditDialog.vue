...
const updatedFolder = (data: {
    path: string;
    basename: string;
}) => {
    if (settingsStore.activeFolderEdit === null) return

    if (settingsStore.activeFolderEdit.name === '' || settingsStore.activeFolderEdit.name === data.basename) {
...
<template>
    <Dialog :open="settingsStore.activeFolderEdit !== null" class="overflow-auto" @update:open="emits('close')">
        <DialogContent class="max-w-xxl sm:max-w-xxl overflow-auto flex flex-col"
            ...
            <FileSelectInput v-model="settingsStore.activeFolderEdit.fullPath" label="Watched folder path"
                :disabled="disabled" @folder:selected="updatedFolder" class="mb-4" />

            <SelectInput label="Upload profile" v-model="settingsStore.activeFolderEdit.profileId" :disabled="disabled"
                :options="settingsStore.profiles.map(profile => ({
                    label: profile.name,
                    value: profile.id
                }))
                    " :disable-clear="true" />
            <TextInput v-model="settingsStore.activeFolderEdit.name" label="Folder name (UI)" :disabled="disabled"
                class="mb-4" />

            <div class="grid grid-cols-2 gap-4">
                <SwitchInput :disabled="disabled" label="Active" v-model="settingsStore.activeFolderEdit.active" />
                <SwitchInput :disabled="disabled" label="Auto approve"
                ...
                <SwitchInput :disabled="disabled" label="Upload subfolders of the watched folder"
                ...
                <SwitchInput :disabled="disabled" label="Upload files directly"
                ...
                <SwitchInput :disabled="disabled" label="Delete uploaded files"
