            ...
            <CardContent>
                <Table :columns="4">
                    <template #head>
                        ...
                        <TableHead class="w-[270px]"># <Button class="w-8" @click="settingsStore.newFolder">
                                <Plus />
                            </Button></TableHead>
                    </template>
                    <template #body>
                        <TableRow v-for="(folder, index) in settingsStore.folders" :key="index">
                            ...
                            <TableCell>
                                <SwitchInput v-model="folder.active"
                                ...
                                <TooltipProvider>
                                    <Tooltip>
                                        <TooltipTrigger as-child>
                                            <Button size="sm" @click="settingsStore.scanFolder(folder.id)" class="me-2">
                                                Scan folder
                                            </Button>
                                        </TooltipTrigger>
                                        <TooltipContent>
                                            <p>This will scan any existing files/folders and place them in the approval
                                                list (or queue depending on settings)</p>
                                        </TooltipContent>
                                    </Tooltip>
                                </TooltipProvider>
                                <Button size="sm" @click="setEditFolder(folder)" class="me-2">Edit</Button>
                                <Button size="sm" @click="settingsStore.deleteFolder(folder.id)"
                                    variant="outline_destructive" class="me-2">Delete</Button>
                            </TableCell>
                        </TableRow>
                    </template>
                </Table>
