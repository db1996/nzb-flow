...
<template>
    <div class="grid grid-cols-1 gap-4">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <RandomSettings :form="form.nameSettings" :useSpecialChars="false" v-if="!useName" :is-name="true" />
            <RandomSettings v-if="!useName" :form="form.passwordSettings" :disabled="skipRarCreation"
                title="Password settings" description="Automatically generate random password for each upload"
                label="Generate random password" labelType="password" :useSpecialChars="true" />
        </div>
        <CardForm title="Other posting Settings"
            ...
            <template #body>
                <div class="grid gap-2 grid-cols-2" v-if="taskSettings">
                    <CopyInput :disabled="disabled" v-model="taskSettings.name" label="Post title"
                        height-class="min-h-[2.5rem]" label-height-class="min-h-[1.5rem]"
                    ...
                    <CopyInput :disabled="disabled" v-model="taskSettings.password" label="Password"
                        height-class="min-h-[2.5rem]" label-height-class="min-h-[1.5rem]"
                        help-class="min-h-[3rem] md:min-h-[2.5rem]" :help="taskSettings.taskSettings.rarSettings.skipRarCreation
                ...
                <div class="grid grid-cols-[1fr_75px_49%] gap-2">
                    <TextInput :disabled="disabled" height-class="min-h-[2.5rem]" label-height-class="min-h-[1.5rem]"
                        label="From" help="The email address to use when posting to usenet. Do not use spaces."
                        v-model="form.post_from" />
                    <SwitchInput :disabled="disabled" v-if="!taskSettings" height-class="min-h-[2.5rem]"
                        label-height-class="min-h-[1.5rem]" label="Randomize from" v-model="form.postFromRandomized" />
                    <div class="grid grid-rows-[2.3rem_2.5rem_1fr] grid-cols-1 gap-0" v-if="taskSettings">
                        <div>
                    ...
                    <TextInput :disabled="disabled" height-class="min-h-[2.5rem]" label-height-class="min-h-[1.5rem]"
                        label="Groups" help="Newsgroups to post to, separated by commas. Do not use spaces"
                        v-model="form.post_groups" />
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <SwitchInput :disabled="disabled" label="Save Rar and par2 files" v-model="form.saveRarPars"
                        help="Keep the RAR and par2 files after upload" />
                </div>
                <hr>
                <div class="grid grid-cols-3 gap-4">
                    <SwitchInput :disabled="disabled" label="Obfuscate subject" v-model="form.obfuscateSubject"
                        help="Obfuscate the subject of the posts to make it harder to identify the content. Will generate a UUID as subject." />
                    <SwitchInput :disabled="disabled" label="Obfuscate filename" v-model="form.obfuscateFilename"
                    ...
                    <SwitchInput :disabled="disabled" label="Obfuscate YENC name" v-model="form.obfuscateYencName"
                        help="Obfuscate the YENC name of the posts to make it harder to identify the content." />
                </div>
                <Alert variant="info">
                    <AlertTitle>Obfuscation</AlertTitle>
                    <AlertDescription>
                        Obfuscation settings will override any custom name/subject/filename settings in the nyuu tab. Enabling all 3 of these options will make it very hard to identify the content being posted. And the only way to download the content is to use the original NZB file. <br />
                        Even then, sometimes usenet downloaders will have issues downloading obfuscated content.
                    </AlertDescription>
                </Alert>
            </template>
        </CardForm>
