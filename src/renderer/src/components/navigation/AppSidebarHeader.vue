        ...
        <div class="flex justify-between gap-2 w-full items-center border-b pb-3 pt-3" v-if="!isCurrent('settings')">
            ...
            <TooltipProvider>
                <Tooltip>
                    ...
                    <TooltipContent>
                        <p  v-if="tasksStore.queueActive">Pausing the queue will pause all queued tasks, currently running uploads will be paused once the current operation is finished</p>
                        <p v-else>Resuming the queue will continue where it left off, if a task is paused in between steps it will just resume</p>
                    </TooltipContent>
                </Tooltip>
            </TooltipProvider>
