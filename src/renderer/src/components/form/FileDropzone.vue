    ...
    <div class="relative grid gap-2">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-3 text-sm">
                <Button variant="ghost" @click="browse"> Browse </Button>
                <Button variant="ghost" :disabled="!items.length" @click="clearAll"> Clear </Button>
            </div>
        </div>
        ...
        >
            ...
            <div
                v-if="loading"
                class="absolute inset-0 z-10 flex items-center justify-center bg-white/60 dark:bg-black/40"
            >
            ...
            <div
                v-if="!hasItems"
                class="text-muted-foreground flex min-h-40 flex-col items-center justify-center gap-2 text-center select-none"
            >
                ...
                <ScrollArea :style="{ height: listHeight }" class="w-full">
                    <ul class="divide-y">
                        <li
                            ...
                            :key="it.id"
                            class="flex flex-col gap-2 px-2 py-2 sm:flex-row sm:items-center sm:gap-3"
                        >
                            <div
                                class="flex min-w-0 flex-1 items-start gap-2 sm:items-center md:gap-3"
                            >
                                ...
                                <div class="min-w-0 whitespace-nowrap">
                                    <span class="mr-2 align-middle">
                                        <Badge
                                            v-if="
                                                !normalizedErrors[index] && it.status === 'accepted'
                                            "
                                            variant="secondary"
                                            class="rounded-sm"
                                        >
                                    ...
                                    <span class="text-muted-foreground ml-2 align-middle text-xs">
                                        • {{ fmt(it.size) }}
                                    </span>
                                    <br />
                                    <span
                                        v-if="normalizedErrors[index] || it.reason"
                                        class="text-destructive block text-xs sm:inline"
                                    >
